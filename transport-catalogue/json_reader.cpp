...
JsonReader::JsonReader(istream& in) : document_(move(Load(in))) {
    request_commands_ = &document_.GetRoot().AsMap().at("base_requests").AsArray();
    settings_commands_ = &document_.GetRoot().AsMap().at("render_settings").AsMap();
    stat_commands_ = &document_.GetRoot().AsMap().at("stat_requests").AsArray();
}
...
void JsonReader::DistributeRequests(TransportCatalogue& transport_catalogue, Array& stop_commands, Array& bus_commands) const {
    for (const auto& request_command : *request_commands_) {
        auto request = request_command.AsMap();
...
void JsonReader::ProcessStopCommands(const Array& stop_commands, TransportCatalogue& transport_catalogue) const {
    for (const auto& stop_command : stop_commands) {
        auto road_distances = stop_command.AsMap().at("road_distances"s).AsMap();
        if (road_distances.size()) {
            for (const auto& [stop, distance] : road_distances) {
                auto first_stop = transport_catalogue.FindStop(stop_command.AsMap().at("name"s).AsString());
...
void JsonReader::ProcessBusCommands(const Array& bus_commands, TransportCatalogue& transport_catalogue) const {
    for (const auto& bus_command : bus_commands) {
        ...
        auto stops_of_bus = bus_command.AsMap().at("stops"s).AsArray();
        ...
        if (stops_of_bus.size() > 1 && !bus_command.AsMap().at("is_roundtrip"s).AsBool()) {
            ...
            transport_catalogue.AddBus({bus_command.AsMap().at("name"s).AsString(), stops, false});
            continue;
        }
        transport_catalogue.AddBus({bus_command.AsMap().at("name"s).AsString(), stops, true});
    }
}
